...
/* post the RDMA write with imm work request */
int perform_rdma_write_with_imm(struct ibv_qp* qp, struct ibv_mr* mr, uint64_t remote_addr, uint32_t rkey) 
{
    struct ibv_sge sge;
    ...
    uint32_t imm_data       = 99;
    wr.wr_id                = 0;
    wr.sg_list              = &sge;
    wr.num_sge              = 1;
    wr.opcode               = IBV_WR_RDMA_WRITE_WITH_IMM;   // RDMA write operation
...
int main (int argc, char* argv[]) 
{
    ...
    if (perform_rdma_write_with_imm(qp, mr, server_addr, server_rkey)) clean_up(-1);

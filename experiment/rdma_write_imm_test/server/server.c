...
static int post_recv_zero(struct ibv_qp *qp)
{
    struct ibv_recv_wr wr  = {0};
    wr.wr_id   = 0xaaaa;      
    wr.num_sge = 0;           

    return ibv_post_recv(qp, &wr, NULL);
}
...
int poll_completion_queue(struct ibv_cq* cq) 
{
    ...
    if (num_completions < 0) { perror("poll CQ"); return -1; } 
    ...
    if (wc.opcode == IBV_WC_RECV_RDMA_WITH_IMM && (wc.wc_flags & IBV_WC_WITH_IMM)) {
        ...
        printf("[INFO] Received IMM = %d\n", imm);
    } else {
        printf("[ERROR] Unexpected WC received (opcode=%d)\n", wc.opcode);
    }
...
int main (int argc, char* argv[])
{
    ...
    if (post_recv_zero(qp)) {
        perror("[ERROR] ibv_post_recv zero-SGE");
        clean_up(-1);
    }
